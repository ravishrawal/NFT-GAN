...
def subprocess_fn(rank, args, temp_dir):
    ...
    for metric in args.metrics:
        ...
        result_dict = metric_main.calc_metric(metric=metric, dataset_1_kwargs=args.dataset_1_kwargs, 
            dataset_2_kwargs=args.dataset_2_kwargs, num_gpus=args.num_gpus, rank=rank, device=device, progress=progress)
        if rank == 0:
...
@click.option('--data2', help='Dataset to evaluate metrics against (directory or zip) [default: same as training data]', metavar='PATH')
@click.option('--gpus', help='Number of GPUs to use', type=int, default=1, metavar='INT', show_default=True)
@click.option('--verbose', help='Print optional information', type=bool, default=True, metavar='BOOL', show_default=True)

def calc_metrics(ctx, metrics, data1, data2, gpus, verbose):
    """Calculate quality metrics for previous training run or pretrained network pickle.
    ...
    if data1 is not None:
        args.dataset_1_kwargs = dnnlib.EasyDict(class_name='training.dataset.ImageFolderDataset', path=data1)
        args.dataset_2_kwargs = dnnlib.EasyDict(class_name='training.dataset.ImageFolderDataset', path=data2)
    ...
    if args.verbose:
        print('Dataset 1 options:')
        print(json.dumps(args.dataset_1_kwargs, indent=2))
        print('Dataset 2 options:')
        print(json.dumps(args.dataset_2_kwargs, indent=2))
